...
class LSTMmodel:
    ...
    def __init__(self, forecastHorizon = 1, 
                        lookBack        = 32, 
                        architecture    = [32], 
                        dropout         = 0.0,
                        regularization  = 0.0,
        ...
        if loadPath is not None:
            ...
            try:    self.lookBack = self.model.layers[0].output_shape[0][1]
            except: self.lookBack = self.model.layers[0].output_shape[1]
            try:    self.noTimeSeries = self.model.layers[0].input_shape[0][2]
            except: self.noTimeSeries = self.model.layers[0].input_shape[2]
    ...
    def createModel(self, noTimeSeries = 1, gpuOptmized = False):
        ...
        def LSTMCell(nodes, **kwargs):
            ...
            def cpuLSTM(nodes, **kwargs):
                return LSTM(nodes,      kernel_initializer      = 'random_uniform',
    ...
    def train(self, epochs, data, 
        ...
        history = self.model.fit(data.xTrainLSTM(), data.yTrainLSTM(), 
                            epochs      = epochs, 
    ...
    def multiStepAheadForecast(self, data, forecastHorizon, 
                                presentIndex, windowMode, windowSize):
        ...
        if windowMode.upper() == "EXPANDING":
            ...
            self.model.fit(data.xTrainLSTM(), data.yTrainLSTM(), 
                                                epochs = 100, 
        ...
        def recursiveMultipleDaysAheadForecast(daysAhead, forecasts):
            ...
            backlog = np.concatenate([data.xTrainLSTM()[-1:, forecasts.shape[1]:], 
                                                            forecasts[:,-self.lookBack:]], axis = 1)
            assert backlog.shape[1] == self.lookBack

            testPredict = self.model.predict(backlog)

            forecasts = np.concatenate([forecasts, np.expand_dims(testPredict, axis = 0)], axis = 1)

            return recursiveMultipleDaysAheadForecast(daysAhead + 1, forecasts)\
                 if (daysAhead < forecastHorizon) else forecasts[0]
...
def searchOptimalParamters():
    ...
    hyperParameterSpace = list(cartProduct(noLayers, 
                                            noNodes, 
                                            dropout, 
                                            regularization, 
                                            lookback, 
    ...
    data = dataUtils.loadScaleData(["DJI"], path, fileName, scaleData = True)
    ...
    trainingLog = pd.DataFrame(columns = ['Time', 
                                            'Architecture', 
                                            'Bidirectional', 
                                            'Validation Loss',
                                            'Training Loss',
    ...
    for parameterSet in hyperParameterSpace[345:]:
        ...
        testLSTM = LSTMmodel(forecastHorizon    = 1,
                                lookBack        = lookback, 
        ...
        trainingHistory = testLSTM.train(5000, data, 
                                            modelname = "HyperparamterTest",
                                            saveModel = False)
        ...
        interationNo += 1
        print(strftime("%Y-%m-%d %H:%M:%S", localtime()) + "   " +  str(interationNo) +\
              " / " + str(totalIterations) + "  Epochs: " + str (noEpochs))
        print(str(architecture) + "  " + str(bidirectional) + "  " + str(np.round(validationError, 4)))

        trainingLog.loc[interationNo] = [strftime("%Y-%m-%d %H:%M:%S", 
                                                    localtime()),
                                                    architecture,
                                                    bidirectional,
                                                    validationError,
        ...
        if runningInColab:  saveLogPath = "/content/gdrive/My Drive/Colab Notebooks/" + \
                                                "LSTM_Log/HyperparameterSearchLSTM2.csv"
        else:               saveLogPath  = "./Data/HyperparameterSearchLSTM2.csv"
